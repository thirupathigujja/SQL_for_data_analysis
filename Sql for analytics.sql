...
-- select * from orders
with cte as(select Restaurant_id,Cuisine,count(Order_id) as total_orders
from Orders
group by Restaurant_id,Cuisine)
select * from (select * ,
row_number() over(partition by Cuisine order by total_orders desc) as rnk
from cte) fr
where rnk<=3

-- ques2
-- daily new customer count from launch date 
with cte as(select Customer_code,cast(min(Placed_at) as date) as first_pur_date from orders
group by Customer_code
order by first_pur_date)
select first_pur_date,count(*) as daily_cus_count
from cte group by first_pur_date

-- ques3
-- count all customers who acquired in 2025 and have only one order and not order after that
select Customer_code,count(*) as cus_count
from orders 
where month(placed_at)=1 and Customer_code not in (select distinct Customer_code from orders
where not month(placed_at)=1)
group by Customer_code
having count(*)=1

-- ques4
-- all all customers with no order in last 7 days but were aquired one month ago with use of promo code

with cte as(select Customer_code,max(placed_at) as latest_purchase,min(placed_at) as first_purchase
from orders
group by customer_code)
select cte.*,o.Promo_code_Name
from  cte
join orders o 
on cte.Customer_code=o.customer_code and cte.first_purchase=o.placed_at
where o.Promo_code_Name is not null and latest_purchase<date_sub(now(),interval 7 day)
and first_purchase<date_sub(now(),interval 1 month)

-- ques 5
-- team is planning to trigger a communication mail for every third order a customer made

select * from(select customer_code,placed_at,row_number() over(partition by customer_code order by placed_at) as rw_num
from orders) fr
where rw_num%3=0 and cast(placed_at as date)=curdate()


-- ques6
-- all cus placed more than 1 order
with cte as (select Customer_code,count(*) as order_count
from orders 
group by Customer_code
having count(*)>1)
select cte.*,count(s.Promo_code_Name)as promo_count from cte
join orders s
on cte.Customer_code=s.Customer_code
where s.Promo_code_Name is not null
group by Customer_code,order_count
having order_count=count(s.Promo_code_Name)

-- ques 7
with cte as( select Customer_code,min(Placed_at) as first_order
from orders
where month(Placed_at)=1 and Promo_code_Name is null
group by Customer_code),
cte2 as(select distinct Customer_code
from orders
where month(Placed_at)=1
)
SELECT 
  ( 
    (SELECT COUNT(*) FROM cte) * 100.0 / 
    (SELECT COUNT(Customer_code) FROM cte2)
  ) AS percentage_count;
